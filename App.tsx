...
const AzkarSection = ({ theme, isDarkMode, mode = 'full', showQuiz, showPlan, setShowPlan, apiKey, onOpenQuiz, onOpenSelfTest, isAdmin, fontScale }: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <AnimatePresence>{showPlan && <SectionPlanModal onClose={() => setShowPlan(false)} theme={theme} isDarkMode={isDarkMode} planData={mode === 'madrasa' ? PLAN_DATA.madrasa : PLAN_DATA.azkars}/>}</AnimatePresence>
      ...
      <div className="grid gap-4">
        {listToShow.map((azkar, i) => (
          <React.Fragment key={i}>
            <CardItem title={azkar.title} data={azkar} theme={theme} isDarkMode={isDarkMode} apiKey={apiKey} isAdmin={isAdmin} fontScale={fontScale} />
          </React.Fragment>
        ))}
      </div>
    </div>
          ...
          {activeTab === 'azkars' && (
            <SectionWrapper title={azkarMode === 'madrasa' ? "Уроки Медресе" : "Азкары (Полный список)"} onBack={() => setActiveTab('home')} action={renderActions('azkars')}>
              <AzkarSection 
                ...
                mode={azkarMode} 
                showQuiz={activeSectionState.azkars.quiz} 
                showPlan={activeSectionState.azkars.plan} 
                setShowPlan={() => toggleSectionFeature('azkars', 'plan')} 
                apiKey={userApiKey} 
                onOpenQuiz={() => setActiveQuiz(azkarMode === 'madrasa' ? 'madrasa' : 'azkars')} 
                onOpenSelfTest={() => setSelfTestData({ 
                    title: azkarMode === 'madrasa' ? "Самопроверка: Уроки Медресе" : "Самопроверка: Азкары", 
                    data: azkarMode === 'madrasa' 
                        ? MADRASA_LESSONS.map(l => ({ question: l.title || "", answer: l.ar || "" }))
                        : [...ALL_MORNING_AZKARS.map((a, i) => ({ question: `Утренний азкар №${i + 1}`, answer: a.ar! })), ...ALL_EVENING_AZKARS.map((a, i) => ({ question: `Вечерний азкар №${i + 1}`, answer: a.ar! }))] 
                })} 
                isAdmin={isAdminMode} 
                fontScale={fontScale} 
              />
