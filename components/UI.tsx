...
export const AccordionItem = ({ title, content, theme, apiKey, fontScale = 1 }: { title: string, content: string, theme: Theme, apiKey?: string, fontScale?: number }) => {
  ...
  const isAlphabetLesson = title === "Урок №1 | Арабский алфавит";
        ...
        {isOpen && (
          <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
            <div className="p-6 pt-0 border-t border-white/5 mt-2">
              {isAlphabetLesson ? (
                <div className="grid grid-cols-[auto_1fr] items-center gap-x-6 gap-y-4" style={{ fontSize: `${1 * fontScale}rem` }}>
                  {content.split('\n').map((line, index) => {
                    const parts = line.split(' - ');
                    if (parts.length !== 2) return null;
                    const [letter, name] = parts;
                    return (
                      <React.Fragment key={index}>
                        <div className="text-right font-serif text-3xl font-bold text-emerald-400" dir="rtl">{letter}</div>
                        <div className="text-left text-lg font-medium text-white/90">{name}</div>
                      </React.Fragment>
                    );
                  })}
                </div>
              ) : (
                <div className="opacity-70 leading-relaxed whitespace-pre-wrap" style={{ fontSize: `${0.875 * fontScale}rem` }}>
                  {content}
                </div>
              )}
            </div>
