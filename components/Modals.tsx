...
import { FIQH_DATA, SURAHS_DATA, TAJWEED_DATA, QUIZ_DATA, MADRASA_LESSONS } from '../data';
...
export const UniversalQuizModal = ({ onClose, theme, isDarkMode, section }: { onClose: () => void, theme: Theme, isDarkMode: boolean, section: string }) => {
  ...
  useEffect(() => {
    ...
    } else if (section === 'madrasa') {
        const shuffledItems = [...MADRASA_LESSONS].sort(() => 0.5 - Math.random());
        generatedQuiz = shuffledItems.map(item => {
            const others = MADRASA_LESSONS.filter(i => i.id !== item.id);
            const distractors = others.sort(() => 0.5 - Math.random()).slice(0, 3).map(i => i.ar || "");
            const allOptions = [...distractors, item.ar || ""].sort(() => 0.5 - Math.random());
            return {
                questionText: item.title || "",
                options: allOptions,
                correctIndex: allOptions.indexOf(item.ar || "")
            };
        });
             ...
             <div className="mb-6 pr-12">
               <div className="flex items-center justify-between mb-4">
                 <h3 className="text-xl font-bold flex items-center gap-2 capitalize">
                    ...
                    {isFiqh ? "ФикъхӀ: Полный тест" : (section === 'madrasa' ? "Уроки Медресе" : `Тест: ${section}`)}
                 </h3>
                 ...
                 {currentQ.options.map((opt, idx) => {
                   ...
                   if (isFiqh) {
                      ...
                      if (selectedOption !== null) {
                       ...
                       if (section === 'madrasa') {
                           btnClass += " font-serif text-right text-lg leading-loose";
                       }
        ...
        {!showResult ? (
          <div className="flex flex-col h-full overflow-hidden">
           ...
           <div className="flex flex-col items-center justify-center text-center py-6 h-full">
             ...
             <p className="text-sm opacity-50 uppercase tracking-widest font-bold mb-6">Тест завершен</p>
             <div className="grid grid-cols-2 gap-4 w-full max-w-sm mb-8">
                <div className={`p-4 rounded-2xl ${isDarkMode ? 'bg-white/5' : 'bg-slate-50'}`}>
                   <div className="text-emerald-500 font-black text-2xl">{knownCount}</div>
                   <div className="text-[10px] opacity-50 uppercase">Знаю</div>
                </div>
                <div className={`p-4 rounded-2xl ${isDarkMode ? 'bg-white/5' : 'bg-slate-50'}`}>
                   <div className="text-orange-500 font-black text-2xl">{unknownCount}</div>
                   <div className="text-[10px] opacity-50 uppercase">Не знаю</div>
                </div>
             </div>
             <button onClick={onClose} className={`w-full py-4 rounded-2xl font-bold uppercase tracking-widest ${theme.buttonPrimary}`}>Завершить</button>
           </div>
        )}
      </motion.div>
    </div>
